    ...
    return <>
        <Col xs="12">
            <Swiper effect="fade" autoplay={{ delay: 3000 }}>
                {props.products.map((product, idx) => (<SwiperSlide key={idx}>
                    <div className={`${styles.back}`} style={{
                        backgroundImage: `linear-gradient(#071832, rgba(0,0,0,0.3), #071832), url(${product.back})`}}>

                        <h1 className="text-title text-center mt-3" style={{fontWeight: "700"}}>
                            <span>Дым</span>
                            <span style={{color: "#FFD600"}}>ОК</span>
                        </h1>
                        <div style={{width: "100%"}}>
                            <a href="tel:+77472562940">
                                <p className={`mt-2 text-default ${styles.footer_phone}`}>+7 747 256 2940</p>
                            </a>
                        </div>
